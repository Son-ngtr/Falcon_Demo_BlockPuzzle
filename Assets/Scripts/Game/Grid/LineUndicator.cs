...
public class LineUndicator : MonoBehaviour
{
    ...
    // square_data store the 1st, 2nd, 3rs square on the board
...
*/    public int[] columnIndexes = new int[9]
    ...
    private (int, int) GetSquarePosition(int square_index)
    {
        ...
        //get postion of square < if square_index = 13 -> (2, 4) >
        return (pos_row, pos_col);
    }

    // return vertical line <if index = 30 -> get ver line 1st, 41 -> line 6>
    ...
    public int GetGridSquareIndex(int square)
    {
        for (int row = 0; row < 9; row++)
        {
            for (int col = 0; col < 9; col++)
            {
                ...
                {
                    // return which row the index is in ~ no of square
                    return row;
                }
            }
        }
